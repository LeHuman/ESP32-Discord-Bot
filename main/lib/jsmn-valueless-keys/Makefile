...
test: test_default test_nonstrict test_lowmem test_nonstrict_lowmem test_permissiveprims test_permissivestrs test_primkeys test_valuelesskeys test_single test_nonstrict_single
...
test_primkeys: test/tests.c jsmn.h
	...
	./test/$@
test_valuelesskeys: test/tests.c jsmn.h
	$(CC) -DJSMN_VALUELESS_KEYS=1 $(CFLAGS) $(LDFLAGS) $< -o test/$@
	./test/$@
...
clean:
	...
	rm -f simple_example
	rm -f jsondump
	rm -f test/test_default test/test_nonstrict test/test_lowmem test/test_nonstrict_lowmem test/test_permissiveprims test/test_permissivestrs test/test_primkeys test/test_valuelesskeys test/test_single test/test_nonstrict_single
